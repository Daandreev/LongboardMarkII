...
int main()
{
	...
	messageWrite[1] = 0b00001111;
    messageWrite[2] = 0b10101010;

	//Send (keep trying until successful - slave may not be ready the first time)
	unsigned char fail = 1;
    
    while(!i2cMTransceive(messageWrite, 3)); // loop shouldnt exit but is used for debug in case slave is retarded
	//{
